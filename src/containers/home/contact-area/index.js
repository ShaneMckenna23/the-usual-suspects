      ...
      <Container>
        <Row>
          <Col lg={4}>
            <MailChimpText>
              {mailchimp.title && (
                <SectionTitle {...sectionTitleStyle} title={mailchimp.title} />
              )}
          ...
          <Col lg={{ size: 7, offset: 1 }}>
            <CtaWrapper>
              ...
              <CtaContent>
                {cta.title && (
                  <Heading {...ctaHeading} className="cta-heading">
                    {parse(cta.title)}
                  </Heading>
                )}
                {cta.desc && <Text {...ctaText}>{cta.desc}</Text>}
                {cta.path && (
                  <Button {...ctaBtn} to={cta.path} icon={<FiChevronRight />}>
                    Connect With me
                  </Button>
                )}
              </CtaContent>
